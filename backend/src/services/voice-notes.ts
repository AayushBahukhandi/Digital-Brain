...
export class VoiceNotesService {
  private static readonly RECORDINGS_DIR = path.join(process.cwd(), 'voice-recordings');
  ...
  ): Promise<{ success: boolean; voiceNote?: VoiceNote; error?: string }> {
    try {
   ...
   */
  private static async convertAudioToText(audioPath: string): Promise<string> {
   ...
   */
  private static async checkRecordingCapabilities(): Promise<boolean> {
   ...
   */
  static async deleteAudioFile(filename: string): Promise<boolean> {
    try {
      const filePath = path.join(this.RECORDINGS_DIR, filename);
   ...
   */
  static async cleanupOldRecordings(maxAgeHours: number = 48): Promise<void> {
    try {
