...
voiceNotesRoutes.post('/:id/followup', upload.single('audio'), async (req, res) => {
    try {
        ...
        const result = await VoiceNotesService.addFollowupRecording(id, req.file.buffer, req.file.originalname);
...
voiceNotesRoutes.post('/meetings/:meetingId/recordings', upload.single('audio'), async (req, res) => {
    try {
        ...
        const result = await VoiceNotesService.addToMeeting(meetingId, req.file.buffer, req.file.originalname, isFollowup === 'true');
        if (result.success) {
            const response = {
                success: true,
